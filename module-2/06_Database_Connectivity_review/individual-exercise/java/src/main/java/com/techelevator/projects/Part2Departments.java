...
public class Part2Departments
{
	...
	private void addDepartment(int departmentId, String departmentName)
	{
		...
		} catch (Exception e)
		{
			...
			System.err.println(
					"There was an error adding the following department --> " + departmentId + ": " + departmentName);
		}
	...
	private void getAllDepartments()
	{
		...
		while (rows.next())
		{
			...
			// --> department_id is the column name in the database IT MUST MATCH what is in
			// the database
	...
	private void getDepartmentById(int departmentId)
	{
		...
		// use jdbcTemplate to execute the statement (run the query) - make sure to
		// include the id
		SqlRowSet rows = jdbcTemplate.queryForRowSet(sql, departmentId);

		if (rows.next())
		{
			// get the values from the columns (we don't need the departmentId - we already
			// know it)
			String departmentName = rows.getString("name");
	...
	private void searchForDepartmentByName(String name)
	{
		...
		// vvv Gregor's debug code. Prints out
		// the sql statement to see why it wasn't working
	...
	private void updateDepartment(int departmentId, String departmentName)
	{
		...
		{
			System.err.println(
					"There was an error updating the following department --> " + departmentId + ": " + departmentName);
		}

...
public class AuctionControllerTest
{
	...
	{
		mvc.perform(get("/auctions/99").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	{
		Auction auction = new Auction("Standing Desk", "Stand up desk to help you stretch your legs during the day.",
				"Johnnie34", 350.00);
	...
	public void searchByTitleShouldReturnList() throws Exception
	{
		...
		MvcResult mvcResult = mvc.perform(get("/auctions?title_like=watch").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();

		List<Auction> searchResults = mapper.readValue(mvcResult.getResponse().getContentAsString(),
	...
	{
		mvc.perform(get("/auctions?title_like=abcsdfsdf").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(content().string("[]"));
	}
	...
	public void searchByPriceShouldReturnList() throws Exception
	{
		...
		MvcResult mvcResult = mvc.perform(get("/auctions?currentBid_lte=70").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
	...
	{
		mvc.perform(get("/auctions?currentBid_lte=1").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(content().string("[]"));
	}
	...
	public void searchByTitleAndPriceExpectOne() throws Exception
	{
		...
		// Act
		MvcResult mvcResult = mvc.perform(get("/auctions" + queryString).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		List<Auction> searchResults = mapper.readValue(mvcResult.getResponse().getContentAsString(),
				new TypeReference<List<Auction>>()
				{

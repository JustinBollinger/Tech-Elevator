...
public class AuctionControllerTests
{
	...
	public void step4_AllMethods_ExpectUnauthorized() throws Exception
	{
		...
		mvc.perform(post("/auctions").contentType(MediaType.APPLICATION_JSON).content(toJson(auction)))
				.andExpect(status().isUnauthorized());
		mvc.perform(put("/auctions/1").contentType(MediaType.APPLICATION_JSON).content(toJson(auction)))
				.andExpect(status().isUnauthorized());
	...
	public void step5_CreateMethod() throws Exception
	{
		...
		mvc.perform(post("/auctions").contentType(MediaType.APPLICATION_JSON).content(toJson(auction))
				.header("Authorization", "Bearer " + creatorToken)).andExpect(status().isCreated());
	...
	public void step5_UpdateMethod() throws Exception
	{
		...
		mvc.perform(put("/auctions/1").contentType(MediaType.APPLICATION_JSON).content(toJson(auction))
				.header("Authorization", "Bearer " + creatorToken)).andExpect(status().isOk());
	}
	...
	public void step5_DeleteMethod() throws Exception
	{
		...
		mvc.perform(delete("/auctions/1").contentType(MediaType.APPLICATION_JSON).header("Authorization",
				"Bearer " + adminToken)).andExpect(status().isNoContent());

		final String creatorToken = getTokenForLogin("creator", "password", mvc);

		mvc.perform(delete("/auctions/2").contentType(MediaType.APPLICATION_JSON).header("Authorization",
				"Bearer " + creatorToken)).andExpect(status().isNoContent());
	}
	...
	public void step6_WhoAmI() throws Exception
	{
		...
		mvc.perform(get("/auctions/whoami").header("Authorization", "Bearer " + token)).andExpect(status().isOk())

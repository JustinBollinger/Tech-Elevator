...
public class AuctionService
{
	...
	{
		// 1. create the Auction object -- the body
		Auction auction = makeAuction(auctionString);
		
		// 2. call API
		// build url -> "http://localhost:3000/auctions" + ...
		String url = API_URL;
		
		// create the header (using the helper function)
		makeEntity(auction);
				
		try
		{
			// 3. execute the request
			Auction auctionResponse = restTemplate.postForObject(url, makeEntity(auction), Auction.class);
			// 4. return the Auction object
	...
	public Auction update(String auctionString)
	{
		...
		// API_URL -> "http://localhost:3000/auctions"
		String url = API_URL + "/" + auctionToUpdate.getId();
		
		// create headers (using helper function)
		makeEntity(auctionToUpdate);
		
		// must return an Auction object
		try
		{
			restTemplate.put(url, makeEntity(auctionToUpdate));

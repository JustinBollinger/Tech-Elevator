<template>
  <div class="card" v-bind:class="{ read: book.read }">
    <h2 class="book-title">
        <router-link v-bind:to="{ name: 'details', params: {isbn: book.isbn}}">{{ book.title }}</router-link>
    </h2>
    <img v-if="book.isbn" v-bind:src="'http://covers.openlibrary.org/b/isbn/' + book.isbn + '-M.jpg'" />
    <h3 class="book-author">
        <router-link v-bind:to="{ name: 'details', params: {isbn: book.isbn}}">{{ book.author }}</router-link>
    </h3>

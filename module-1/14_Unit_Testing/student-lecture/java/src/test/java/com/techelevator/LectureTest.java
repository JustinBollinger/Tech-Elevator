...
public class LectureTest
{
	...
	/*
	 * If a method is annotated with @Before, it will be executed immediately prior
	 * to every test. It is intended to be used when there is a repetitive setup
	 * (i.e. "Arrange") task that is performed by several tests
	 */
	@Before
	public void setup()
	{
	...
	/*
	 * If a method is annotated with @After, it will be executed immediately after
	 * every test. It is intended to be used when there is a repetitive cleanup task
	 * that is performed by several tests (e.g. deleting temp files, rolling back
	...
	/*
	 * Each test is implemented as a method with the @Test annotation. When the
	 * JUnit framework is invoked, it looks for these @Test annotations on the test
	 * class and runs such methods as tests.
	 *
	 ...
	 * should not be construed as a general standard or convention. You can also use
	 * camelCase for test method names. You should follow the norm at whatever
	 ...
	 *
	 * Test methods always: - are public - return void - take no arguments
	 */
	@Test
	public void length_returns_the_number_of_characters_in_a_String()
	{
		...
		/*
		 * The assertEquals method validates that two values are equal and fails the
	...
	public void startsWith_returns_true_if_a_string_starts_with_the_specified_characters()
	{
		...
		/*
		 * The assertTrue method validates that the boolean value provided as an
		 * arugment is true and fails the test if it is false.
		 */
		...
		/*
		 * every assert method allows the first parameter to be a String that contains a
		 * message that should be displayed when the assertion fails
		 *
		 * This is particularly helpful with assertTrue as otherwise the failure output
		 * would simply state "Expected: true Actual: false", which sometimes isn't much
	...
	{
		System.out.println("this_test_fails_every_time"); // FOR DEMONSTRATION PURPOSES ONLY, don't do this in your own
															// tests
